...
function loadEtf() {
  ...
  const current =
    Number(etfStock.current || 0);

  const previous =
    Number(etfStock.previous || 0);

  const open =
    Number(etfStock.open || 0);

  const high =
    Number(etfStock.high || 0);

  const low =
    Number(etfStock.low || 0);

  const volume =
    Number(etfStock.volume || 0);
  ...
  etfTable.innerHTML = `
    <tr
      class="clickable-row"
      onclick="goDetail('${etfStock.code || ETF_CODE}')"
    >
      ...
      <td class="etf-name">
        ${etfStock.name || "KODEX 차이나휴머노이드로봇"}
      </td>

      <td>
        <span class="badge">
          ${etfStock.code || ETF_CODE}
        </span>
      </td>
  ...
  stocks.forEach((stock, index) => {
    ...
    tbody.innerHTML += `
      ...
      >
        ...
        <td class="${cls}">
          ${stock.diff > 0 ? "+" : ""}
          ${formatNumber(stock.diff)}
        </td>

        <td class="${cls}">
          ${stock.changeRate > 0 ? "+" : ""}
          ${(stock.changeRate * 100).toFixed(2)}%
        </td>
